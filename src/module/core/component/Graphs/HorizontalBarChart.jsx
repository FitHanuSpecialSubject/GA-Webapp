...
export default function HorizontalBarChart({ data, title }) {
  ...
  // Calculate "Mean-Min" values (from min to mean)
  const meanMinValues = meanValues.map((mean, index) => mean - minValues[index]);
  
  // Calculate "Max-Mean" values (from mean to max)
  ...
  const chartData = {
    ...
    datasets: [
      ...
      },
      {
        label: 'Mean-Min',
        data: meanMinValues,
        backgroundColor: 'rgba(241, 196, 15, 0.8)',  // Yellow
